    ...
    <body>
        ...
        <div class="dashboard-container">
            <div class="filter-container" id="add-newuser-container" style="display: flex;">
                <form action="manageChangeService" method="post">
                    <div class="filter-title">
                        <h1>Service Information</h1>
                    </div>
                    <table>
                        <input type="text" name="serId" readonly value="${service.getServiceId()}" style="display: none">
                        <tbody>
                            <tr>
                                <td><label for="ServiceName">Service Name</label></td>
                                <td><input type="text" name="ServiceName" placeholder="Enter service name" id="ServiceName"
                                           required value="${service.getServiceName()}"></td>
                            </tr>
                            <tr>
                                <td><label for="Price">Price</label></td>
                                <td><input type="money" name="Price" placeholder="Enter price..." id="Price"
                                           required value="${service.getPrice()}"></td>
                            </tr>
                            <tr>
                                <td><label for="Description">Description</label></td>
                                <td><input type="text" name="Description" placeholder="Enter Description..." id="Description"
                                           required value="${service.getDescription()}"></td>
                            </tr>
                            <tr>
                                <td><label for="DoctorName">Doctor</label></td>
                                <td>
                                    <select name="doctorId" id="DoctorName">
                                        <c:forEach var="doctorList" items="${doctorList}">
                                            <option value="${doctorList.getDoctorId()}">${doctorList.getLastname()} ${doctorList.getFirstname()} </option>            
                                        </c:forEach>
                                    </select>
                                </td>
                            </tr>
                    ...
                    <div class="add-user-btn-container">
                        <div>
                            <button id="add-user-btn" type="submit" onclick="showUpdateAlert()">Update</button>
                        </div>
                        <div>
